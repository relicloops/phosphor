...
const sections: Section[] = [
  ...
  },
  {
    ...
    body: () => frag(
      ...
      p( 'All saves are routed to the configured --log-directory (from [serve.neonsignal] manifest or CLI flag). If no log directory is configured, files are written to cwd.' ),
      p( 'V (selection export): Saves selected lines to <log_dir>/phosphor.<panel>.json. Daily-indexed with a DDMMYYYY date key. Lines are numbered sequentially. Appends to existing file content. ANSI stripped.' ),
      p( ':save <path>: Saves the entire focused panel ring buffer to <log_dir>/DD.MM.YYYY.<panel>.<path>.json. Uses incrementing numbered slots. The panel is cleared after each save.' ),
      p( ':saveall: Saves all panels to <log_dir>/DD.MM.YYYY.all.json. Same slot structure but each slot contains sub-objects keyed by panel name. All panels are cleared after saving.' ),
  ...
  },
  {
    ...
    body: () => frag(
      p( 'Press g to open the fuzzy log finder. It recursively scans cwd and subdirectories for .json files and presents a searchable file list as a popup overlay. The log directory tree (--log-directory) is always included. Built-in excludes skip node_modules, build, public, subprojects, certs, .venv, __pycache__, .cache, .git. Additional excludes from .gitignore and [fuzzy].exclude in the manifest are also applied.' ),
      table(
        [ 'Key', 'Action' ],
        [
    ...
    body: () => frag(
      ...
      table(
        ...
        [
          ...
          [ 'Ctrl+P', 'Any', 'New shell view (up to 4)' ],
          [ 'Ctrl+G', 'Any', 'Focus back to shell' ],
          [ 'Ctrl+B', 'Any', 'Next shell view tab' ],
          [ 'Ctrl+R', 'Any', 'Previous shell view tab' ],
          [ 'Ctrl+Q', 'Any', 'Close shell (kill all)' ],
          [ 'Ctrl+D', 'Shell', 'Open phosphor command bar' ],
          [ 'Ctrl+N', 'Shell', 'Cycle to next screen' ],
          [ 'Ctrl+X', 'Shell', 'Minimize focused screen' ],
          [ 'Ctrl+W', 'Shell', 'Save screen output to JSON' ],
  ...
  },
  {
    ...
    body: () => frag(
      ...
      table(
        [ 'Key', 'Action' ],
        [
          [ 'Ctrl+P', 'Create new shell view (up to 4)' ],
          [ 'Ctrl+G', 'Focus back to shell from normal mode' ],
          [ 'Ctrl+B', 'Next shell view tab' ],
          [ 'Ctrl+R', 'Previous shell view tab' ],
          ...
          [ 'Ctrl+N', 'Cycle to next screen' ],
          [ 'Ctrl+X', 'Minimize focused screen overlay' ],
          [ 'Ctrl+W', 'Save screen output to <log_dir>/shell/DD.MM.YYYY.shell.json' ],
      ...
      p( 'Screens can be minimized (hidden but still running). Ctrl+W saves the focused screen\'s output as JSON to <log_dir>/shell/DD.MM.YYYY.shell.json with incremental numbered slots. The :shell and :shellclose commands (via Ctrl+D) also open and close the shell.' ),
    ),
  },
];
